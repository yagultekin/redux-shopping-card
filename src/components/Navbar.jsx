...
const Navbar = () => {
  ...
  useEffect(() => {
    window.addEventListener("scroll", () => {
      // this is another function for the scroll (used also in fitness app).
      // This one is much more clean though.
    ...
    <div className={`${scroll ? "bg-grey shadow-lg" : ""} fixed top-0 left-0 w-full z-20`}>
        <div className='flex items-center justify-between relative container py-4 px-2 mx-auto'>
            ...
            <div className='relative cursor-pointer' onClick={() => dispatch(open())}>
                ...
                <div className='absolute w-4 h-4 rounded-full z-10 right-[-3px] bottom-[-3px] flex items-center justify-center text-[10px] bg-black text-white'>{amount}</div>
            </div>
        </div>
    </div>
  )
